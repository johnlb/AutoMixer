...
x1_rms = log( abs(x1(piece_win))./thresh1 );
...
x3_rms = log( abs(x3(piece_win))./thresh3 );
...
% % Try fitting data
% a = log(alpha2(piece_win,1)./amax(1));

% time3 = (0:length(piece_win)-1)'*ts;
% [a_ amax_ tau_] = find_firstord(time3,a,.01);
...
sys = ssest(data{1},2,'InitialState','estimate')
ss_sys = ss(sys);


fprintf('simulating system...');
time3 = (0:length(piece_win)-1)*ts;
a_ = lsim(ss_sys, v(:,1),time3);
...
figure(7);
subplot(211);
plot(time3,a(:,1),time3,a_);
subplot(212);
plot(time2,alpha2(:,1),time3,alpha_);