...
% %without noise
% x_rms_without_noise = horzcat(x_rms, zeros(L,1));
% %constant error
...
% M = 3000;
% [A1, B1] = cal_plot(x_rms_without_noise, t_rms, M, 1, 'without noise term');
%[A2, B2] = cal_plot(x_rms_error, t_rms, M, 0, 'with constant noise term');
%[A3, B3] = cal_plot(x_rms, t_rms - wgn(L,2, p), M,1, 'with white noise term');
%[A4, B4] = cal_plot(x_rms_boot, t_rms,M,0, 'with bootstrap noise term');
%[A5, B5] =cal_plot(x_rms, t_rms_boot,M,0, 'substract noise term from output');
% figure
% plot(A1(2,:), 'r-');
% hold on 
% plot(A2(2,:)),'g-';
